...
local essential = function(cfg)
    ...
    table.insert(
        cfg.keys,
        { mods = "CTRL", key = "c", action = copy_if_select }
    )
    table.insert(cfg.keys, {
        mods = "CTRL|SHIFT",
        key = "C",
...
local pane_input = function(cfg)
    ...
    table.insert(cfg.keys, {
        mods = "CTRL|SHIFT",
        key = ":",
        action = act.SplitVertical(split),
    })
    table.insert(cfg.keys, {
        mods = "CTRL|SHIFT",
        key = '"',
        action = act.SplitHorizontal(split),
    })
    table.insert(cfg.keys, {
        mods = "CTRL|SHIFT",
        key = "Z",
    ...
    local nvim_scrollback = wezterm.action_callback(function(window, pane)
        ...
        -- ... into temporary file
        local temp_file   = "/tmp/"
            .. os.date("%Y-%m-%d_%H-%M-%S_wezterm-scrollback.log.ansi")
